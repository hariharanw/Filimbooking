...
//	Dashboard page
	...
	public String getUser(@RequestParam("movieName") String movieName, HttpSession session, Model m) {
		List<MovieDetails> movie2 = dao.getAllMovie();
		List<String> checkMovie = new ArrayList<>();
		 List<String> selectedSeats = (List<String>) session.getAttribute("selectedSeats_" + movieName);
		 if (selectedSeats == null) {
		        selectedSeats = new ArrayList<>();
		        session.setAttribute("selectedSeats_" + movieName, selectedSeats);
		    }
		...
		if (checkMovie.contains(movieName)) {
			...
			m.addAttribute("selectedSeats", selectedSeats);
	        m.addAttribute("availableSeats", seatNo1);
...
//	Seat booking process
	...
	public String bookSeat(@ModelAttribute("Seat") Seat seat, @RequestParam("movieName") String movieName,
		 ...
		 if (selectedSeats == null) {
		  ...
		  // Update selected seats for the current movie
		    selectedSeats.clear(); // Clear previous selections
		    selectedSeats.addAll(seat.getSeatNo());
		...
		if (object == null) {
			...
			if (((date.isAfter(currentDate)) || (date.equals(currentDate)))
				...
				session.setAttribute("msg", "your seat book successsfully");
				 sendBookingConfirmationEmail(object, movieName, date.toString(), time, seat.getSeatNo());
	...
	private void sendBookingConfirmationEmail(Customer customer, String movieName, String bookingDate, String bookingTime, List<String> seatNumbers) {
	    String toEmail = customer.getEmail();
	    String subject = "Booking Confirmation";

	    emailService.sendBookingConfirmationEmail(toEmail, subject, customer.getName(), movieName, "Theater Name", bookingDate, bookingTime, String.join(", ", seatNumbers));
	}
